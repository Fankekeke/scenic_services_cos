...
export default {
  ...
  methods: {
    downLoad () {
      ...
      this.$get('/cos/scenic-order/page', { size: 9999, current: 1 }).then((r) => {
        let newData = []
        r.data.data.records.forEach((item, index) => {
          newData.push([(index + 1).toFixed(0), item.scenicName, item.code, item.userName, item.price, item.orderStatus === 1 ? '未使用' : '已销票', item.createDate])
        })
        let spread = newSpread('order1')
        spread = floatForm(spread, 'order1', newData)
        saveExcel(spread, '景区订单.xlsx')
        floatReset(spread, 'order1', newData.length)
